...
# Guard oletools import — missing DLLs or package must not crash the service
HAS_OLETOOLS = False
try:
    ...
    logger.warning(
        f"[office_check] oletools not available — Office macro analysis disabled: {e}"
    )
...
def analyse_office_doc(file_path: str) -> Dict[str, Any]:
    ...
    if not HAS_OLETOOLS:
        results["details"].append(
            "oletools dependency not installed — macro analysis skipped"
        )
        return results
        ...
        if vba_parser.detect_macros():
            ...
            for _, _, _, code in vba_parser.extract_macros():
                ...
                if r.suspicious:
                    ...
                    matches_str = (
                        ", ".join(matches) if matches else "auto-execution or write trigger"
                    )
                    results["details"].append(
                        f"Suspicious macro heuristics matched: {matches_str}"
                    )
