  ...
  return (
    <Tabs defaultValue="overview" className="w-full">
      <TabsList className="bg-dash-input/50 border border-dash-border p-0.5 inline-flex w-fit rounded-lg gap-1">
        <TabsTrigger 
          value="overview" 
          className="font-mono uppercase text-[10px] tracking-wider rounded-md text-dash-muted transition-all px-4 py-1.5 data-[state=active]:bg-dash-card data-[state=active]:text-emerald-600 data-[state=active]:shadow-2xs font-bold"
        >
          Mission Overview
        </TabsTrigger>
        <TabsTrigger 
              ...
              <CardContainer className="space-y-[12px]">
                {recentSubmissions.map((sub, i) => (
                  ...
                  >
                    ...
                    <CardContainer>
                      <p className="font-mono text-sm truncate text-dash-text group-hover:text-emerald-600 transition-colors">
