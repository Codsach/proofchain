  ...
  if (!caseData) {
    return (
      <div className="rounded-3xl border border-white/5 bg-white/[0.01] backdrop-blur-2xl p-20 text-center shadow-2xl">
        <p className="text-white/30 text-sm font-medium">Case subject not found in secure storage.</p>
        <Link href="/admin/cases" className="text-emerald-500 text-xs font-bold uppercase tracking-widest hover:text-emerald-400 mt-4 block transition-colors">
          ← Return to Archives
        </Link>
      </div>
    );
  ...
  return (
    <div className="space-y-8 pb-10">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        <Link href="/admin/cases" className="text-[10px] font-bold uppercase tracking-widest text-white/30 hover:text-emerald-500 transition-colors flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          <motion.div 
            ...
            className="rounded-2xl border border-white/5 bg-white/[0.02] backdrop-blur-xl p-6 shadow-2xl relative overflow-hidden group"
          >
            <div className="absolute top-0 right-0 p-4">
              <div className="text-[10px] font-bold text-emerald-500/20 uppercase tracking-[0.3em] group-hover:text-emerald-500 transition-colors cursor-default">Subject Details</div>
            </div>
            <div className="grid grid-cols-2 gap-8 text-sm pt-4">
              <div className="space-y-1">
                <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest">Incident Category</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
              {caseData.files.map((file, idx) => (
                <motion.div 
                  ...
                  className="rounded-2xl bg-white/[0.02] border border-white/5 hover:border-emerald-500/20 p-5 space-y-4 transition-all group/file hover:shadow-[0_0_20px_rgba(16,185,129,0.05)]"
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <a
                      ...
                      className="text-[10px] font-bold uppercase tracking-[0.2em] text-emerald-500/60 hover:text-emerald-400 transition-colors border border-emerald-500/20 bg-emerald-500/5 px-3 py-1 rounded"
                    >
                      Gateway
                    </a>
                  </div>
                  <div className="space-y-1.5">
                    <p className="text-[10px] font-bold text-white/20 uppercase tracking-widest">Digital Fingerprint</p>
                    <p className="text-[10px] font-mono text-emerald-500/40 break-all bg-black/40 rounded px-2 py-1.5 border border-white/5 font-medium">
                      {file.sha256Hash}
                    </p>
                  </div>
                </motion.div>
        ...
        <div className="space-y-8">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              <h2 className="text-sm font-bold text-white uppercase tracking-[0.2em]">Neural Review</h2>
              <div className="h-px flex-1 bg-white/5" />
            </div>
          ...
          <AnimatePresence>
            {!isLoadingVerdict && verdict && (
              <motion.div 
                ...
                animate={{ opacity: 1, y: 0 }}
                className="rounded-2xl border border-white/5 bg-white/[0.02] backdrop-blur-xl p-6 space-y-4 shadow-xl"
              >
                <div className="flex items-center justify-between">
                  <h2 className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em]">Terminal Verdict</h2>
                  <div
          ...
          <AnimatePresence>
            {!isLoadingTransfers && transfers.length > 0 && (
              <motion.div 
                initial={{ opacity: 0, scale: 0.95 }}
                animate={{ opacity: 1, scale: 1 }}
                className="rounded-2xl border border-white/5 bg-white/[0.01] backdrop-blur-xl p-6 space-y-5 shadow-xl"
              >
                <div className="flex items-center gap-4">
                  <h2 className="text-[10px] font-bold text-white/20 uppercase tracking-[0.2em]">Custody Chain</h2>
                  <div className="h-px flex-1 bg-white/5" />
                  <span className="text-[10px] font-bold text-emerald-500/40">{transfers.length}</span>
                </div>
                <div className="space-y-4 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                  {transfers.slice().reverse().map((t, i) => (
                    <div key={t._id} className="relative pl-6 border-l border-white/5 pb-4 last:pb-0">
                      <div className="absolute left-[-4.5px] top-0 w-2 h-2 rounded-full bg-emerald-500/20 border border-emerald-500/40" />
                      <div className="space-y-1">
                        <p className="text-[11px] font-bold text-white group-hover:text-emerald-400 transition-colors uppercase tracking-tight">
                          {t.fromUserId.fullName} <span className="text-white/20 mx-1">→</span> {t.toUserId.fullName}
                        </p>
                        <p className="text-[10px] text-white/40 font-medium leading-tight">{t.reason}</p>
                        <p className="text-[9px] text-white/20 font-mono italic tracking-tighter">
